> **本仓库是 [zai-org/ZCode](https://github.com/zai-org/ZCode) 的本地化（内网）发行版分支。**
>
> 面向无法访问外部网络的部署环境，相对上游有两类改动。
>
> **移除对外依赖**
>
> - **零出网**：产品端点在客户端层被整体禁用（Local 档位由主进程固定注入开关），遥测端点
>   默认未配置；套餐升级、插件市场、反馈上报、自动更新等联网入口一并移除。
> - **纯本地模型**：模型设置只保留「自定义供应商」，支持 OpenAI / Anthropic 兼容接口，
>   并可从 `/models` 一键拉取可用模型列表。
> - **独立产品身份**：产品名为 **ZCode Local**，可与官方版本并排安装；用户配置、日志与
>   会话库位于独立数据根，互不干扰。
> - **手动触发的发布流水线**：GitHub Actions 构建 Windows / macOS 的命令行运行包与桌面
>   安装包，不配置任何自动触发器，不会因 push 或 tag 消耗构建资源。
>
> **补充上游没有的能力**
>
> - **生成速度显示**：上下文浮窗给出最近几次模型请求的精确 tok/s；思考过程中在「正在思考」
>   后面显示实时估算值（带 `≈`），便于判断本地模型当前的快慢。
>
> 上游的通用说明见下文。
